...
function _hlTxDateRows(date, desc) {
  ...
  matchIds.forEach(function(id){
    ...
    row.style.border = '1px solid rgba(0,245,255,.5)';
    row.style.boxShadow = '';
    row.style.animation = '';
    void row.offsetWidth;
    row.style.animation = '_hlGlow 2.2s ease-out 1';
    row.addEventListener('animationend', function(){ row.style.boxShadow = '0 0 0 1px rgba(0,245,255,.4), var(--g-shadow)'; }, { once: true });
  });
...
function txDetailModal(id) {
  ...
  // ── Hero logo (vendor > account > type emoji) ───────────────
  var heroLogoHtml = '';
