...
function toggleTrendNetItem(desc, idx) {
  var i = _trendNetSelItems.indexOf(desc);
  if (i > -1) { if (_trendNetSelItems.length > 1) _trendNetSelItems.splice(i, 1); }
  else { _trendNetSelItems.push(desc); }
  var picker = document.getElementById('trendNetCatPicker');
  if (picker) picker.querySelectorAll('button[data-item]').forEach(function(btn){
    var bd = btn.getAttribute('data-item'), bi = parseInt(btn.getAttribute('data-idx'), 10);
    var col = PALETTE[bi % PALETTE.length], on = _trendNetSelItems.indexOf(bd) > -1;
    btn.style.border = '1px solid '+(on ? col : 'transparent');
...
function _renderTrendNetChart(_cycleDb, cycle) {
  ...
  var datasets = _trendNetSelItems.map(function(desc){
    var fi = 0;
    _trendNetFreqItems.forEach(function(o, j){ if (o.desc === desc) fi = j; });
    var col = PALETTE[fi % PALETTE.length];
    return {
      label: desc, borderColor: col, backgroundColor: col+'18',
  ...
  chartTrendNet = new Chart(canvas.getContext('2d'), {
    type: 'line', data: { labels: labelsT, datasets: datasets },
    options: {
