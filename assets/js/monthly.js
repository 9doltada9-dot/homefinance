...
function renderMonthly(){
  ...
  var incomeBlock = (function(){
    ...
    return ic.length?'<table><tr><th>ประเภท</th><th style="text-align:right">จำนวน</th></tr>'+
      ic.map(function(pair){var k=pair[0],v=pair[1];return '<tr><td>'+k+'</td><td style="text-align:right;font-family:monospace;color:var(--green)">'+fmt(v)+'</td></tr>';}).join('')+'</table>':'<div class="empty">ไม่มีรายรับ</div>';
  })();
  ...
  document.getElementById('monthlyContent').innerHTML=
    ...
    '<div class="grid2">'+
      '<div class="card">'+
        ...
        catEntries.map(function(pair){var cat=pair[0],amt=pair[1];return '<tr><td>'+cat+'</td><td style="text-align:right;font-family:monospace">'+fmt(amt)+'</td>'+
